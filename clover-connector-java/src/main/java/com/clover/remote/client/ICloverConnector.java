...
public interface ICloverConnector extends Serializable {
   ...
   * entered.
   *
   * @param request The accepted VerifySignatureRequest the device passed to 
   ...
   * method accepts the Payment. A Challenge may be triggered by a potential duplicate 
   * Payment or an offline Payment.
   *
  ...
  /**
   * If Payment confirmation is required during a transaction due to a Challenge, this 
   * method rejects the Payment. A Challenge may be triggered by a potential duplicate 
   * Payment or an offline Payment.
   *
   ...
   * PreAuthResponse.
   *
   * @param The CapturePreAuthRequest details.
   */
   ...
   * Voids a transaction.
   *
   * @param request A VoidRequest object containing basic information needed to void the 
   * transaction.
   */
   ...
   *
   * <p>
   * <b>Note:</b> The merchant account must be configured to allow transaction closeout.
   *
  ...
  /**
   * Sends a print request using the PrintRequest object. Used to print text, Bitmap image 
   * objects, and images from a URL using the specified printer.
   * 
